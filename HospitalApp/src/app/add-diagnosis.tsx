...
export default function AddDiagnosisScreen() {
  ...
  const handleSubmit = async () => {
    if (!diagnosis.trim()) { setError('Diagnosis details are required.'); return; }
    setError(''); setLoading(true);
    try {
      await api.put(`/patient/diagnosis/${id}`, { diagnosis: diagnosis.trim() });
      router.back();
    } catch (err: any) {
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.card}>
          ...
          <TextInput
            ...
            placeholder="Enter full diagnosis details..."
          ...
          <View style={styles.actions}>
            ...
            <TouchableOpacity style={[styles.btn, styles.submitBtn]} onPress={handleSubmit} disabled={loading}>
              {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>Submit Diagnosis</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  textArea: { height: 140, textAlignVertical: 'top', marginBottom: 20 },
